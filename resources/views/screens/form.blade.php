...
@section('content')
    ...
    <h1>Add Planni Screen to household</h1>

    <div class="well">

        <form method="POST"
              action="{{ $screen->exists ? route('screens.update', $screen->id) : route('screens.store')  }}">
                    ...
                    <div class="form-group">
                        ...
                        <select name="type" class="form-control">
                            @foreach ($screen_types as $key => $val)
                                <option
                                    value="{{ $key }}" {{ (old("type", $screen->type) == $key ? "selected":"") }}>{{ $val }}</option>
                            @endforeach
                        </select>
                        <p class="help-block">
                            Select the type of screen. Currently only 2 modes are supported. Normal is the default,
                            mirror is a high
            ...
            <div class="btn-group">
                ...
                @else
                    <input type="submit" class="btn btn-raised btn-primary" value="Add screen">
                @endif
